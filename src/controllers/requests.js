...
function getAllCurrentReqeusts(req, res, next){
  ...
  .then(function(result){
    if (!result) {
      return next({ status: 404, message: 'No current requests found'})
    }
    res.status(200).send(result)
  })
}
...
function getCurrentReqeustsByUserID(req, res, next){
  ...
  if (!user_id) {
    return next({ status: 404, message: 'No user id'})
  }
  modelsRequests.getCurrentReqeustsByUserID(user_id)
  .then(function(result){
    if (!result){
      return next({ status: 404, message: 'No request found'})
    }
    res.status(200).send(result)
  })
}
...
function getRequestByUserID(req, res, next){
  ...
  if(!user_id){
    return next({ status: 404, message: 'No user id, cannont get information'})
  }
...
function getRequestsByRequestID(req, res, next){
  ...
  if (!request_id){
    return next({ status: 404, message: 'No request id, cannot get information'})
  }
  modelsRequests.getRequestsByRequestID(request_id)
  .then(function(result){
    if (!result) {
      return next({ status: 404, message: 'No resutls found'})
    }
    res.status(200).send(result)
  })
}
...
function updateRequestByRequestID(req, res, next){
  ...
  if (!description && !status) {
    return next({ status: 404, message: 'No description or status was provided, cannot update request'})
  }
...
function createRequest(req, res, next){
  ...
  let description = req.body.description
  if (!user_ID){
    return next({ status: 404, message: 'No user id, cannot create request'})
  }
  if (!description){
    return next({ status: 404, message: 'No description cannot create request'})
  }
  modelsRequests.createRequest(user_ID, description)
  .then(function(result){
  if (!result) {
    return next({ status: 404, message: 'Request not created'})
  }
  res.status(200).send(result)
  })
}
