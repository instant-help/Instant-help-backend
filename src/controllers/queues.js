...
function getQueueByHelperID(req, res, next){
  ...
  if (!helper_id){
    return next({ status: 404, message: 'No helper id, cannot get queue information'})
  }
...
function getQueueByRequestID(req, res, next){
  ...
  if (!request_id){
    return next({ status: 404, message: 'No request id, cannot get queue information'})
  }
...
function getHelperSessionsFromQueueByHelperID(req, res, next){
  ...
  .then(function(result){
    ...
    res.status(200).send({id: getActiveSessionInfo(result)})
  })
}
...
function createQueue(req, res, next){
  ...
  if (!helper_id || !request_id) {
    return next({ status: 404, message: 'No request id or helper id, cannot post to queue'})
  }
...
function deleteQueueByRequest_id(req, res, next){
  ...
  if(!request_id){
    return next({ status: 404, message: 'No request id, cannot delete to queue'})
  }
